...
def main():
    # Initialize timing results file
    with open('prm_timing_results.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['num_samples', 'sampling_time', 'roadmap_time', 
    ...
    for num_samples in sample_sizes:
        ...
        try:
            # Sample free points
            start = time.time()
            sampled_points = sample_free_points(occupancy_grid, min_bound, voxel_size, num_samples)
            iteration_times['sampling'] = time.time() - start

            # Build roadmap
            start = time.time()
            roadmap = build_prm_roadmap(sampled_points, occupancy_grid, min_bound, voxel_size, k=10)
            iteration_times['roadmap'] = time.time() - start

            # Find path
            start = time.time()
            shortest_path_points = find_shortest_path(roadmap, start_point, goal_point, sampled_points)
            iteration_times['pathfinding'] = time.time() - start

            # Simplify path if found
            if shortest_path_points:
                start = time.time()
                simplified_path = simplify_path(shortest_path_points, occupancy_grid, min_bound, voxel_size)
                iteration_times['simplification'] = time.time() - start
            else:
                print("No path found")
                iteration_times['simplification'] = 0

        except Exception as e:
            print(f"Error with {num_samples} samples: {str(e)}")
            continue
        ...
        # Print results
        print(f"Sampling time: {iteration_times['sampling']:.2f}s")
        print(f"Roadmap construction: {iteration_times['roadmap']:.2f}s")
        print(f"Path finding: {iteration_times['pathfinding']:.2f}s")
        print(f"Path simplification: {iteration_times['simplification']:.2f}s")
        print(f"TOTAL TIME: {total_time:.2f}s")

        # Save to CSV
        with open('prm_timing_results.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
                num_samples,
                iteration_times['sampling'],
                iteration_times['roadmap'],
                iteration_times['pathfinding'],
                iteration_times['simplification'],
                total_time
            ])

        # Visualize only for the first run (optional)
        if num_samples == sample_sizes[0] and shortest_path_points:
            visualize_prm_over_pointcloud(pcd, sampled_points, roadmap, 
                                        shortest_path_points, simplified_path)
